...
public class ExcelUtility {
    //kendisine verilen path deki excel'in istenilen sheet'indeki
    //istenilen kolona kadar bilgisini okuyup Arraylist formatında
    ...
    public static ArrayList<ArrayList<String>> getListData(String path, String sheetName, int columnCount) {
        ...
        } catch (IOException e) {     //IOException ile birçok try catch ihtiyacını tek seferde çözebiliyoruz
            throw new RuntimeException(e);
        }

        Sheet sayfa = workbook.getSheet(sheetName);

        for (int i = 0; i < sayfa.getPhysicalNumberOfRows(); i++) {   //bu kod ile bütün satırları dolaşmış oluyoruz.
            // aşağıdaki kolon sayısı kadar dolaşıyor
            ArrayList<String> satirData = new ArrayList<>();
            for (int j = 0; j < columnCount; j++) {     //kaç kolon alacağımızı columnCount belirliyor
    ...
    //TODO: kendisine verilen path, scenario, browserTipi, zaman parametreleri
    // ile yeni bir excel e bütün raporu yazacak. dosyanın mevcut olup olmadığının
    // kontrolü için google a bakılacak
    ...
    public static void writeExcel(String path, Scenario scenario, String browserName, String time) {
        ...
        //aşağıdaki kısım test etmek için yazıldı, gerekli değil normalde.
